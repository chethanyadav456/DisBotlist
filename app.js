  ...
  const renderTemplate = (res, req, template, data = {}) => {
   ...
   app.get("/login", (req, res, next) => {
    ...
    app.get("/callback", passport.authenticate("discord", { failureRedirect: "/error?code=999&message=We encountered an error while connecting." }), async (req, res) => {
        ...
        client.users.fetch(req.user.id).then(async a => {
        client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({dynamic: true})).setThumbnail(a.avatarURL({dynamic: true})).setColor("GREEN").setDescription(`[**${a.username}**#${a.discriminator}](https://disbotlist.xyz/user/${a.id}) User ** logged into the site **.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
    ...
    app.get("/dc", (req, res) => {
      res.redirect('https://discord.gg/c96hPfyWQd');
    });  
    ...
    app.post("/addbot", checkMaintence, checkAuth, async (req,res) => {
      ...
      client.channels.cache.get(channels.botlog).send(`<:add:845717716559593512> <@${req.user.id}> added \`${a.tag}\` \nhttps://disbotlist.xyz/bot/${rBody['botID']}/`)
    ...
    app.post("/bot/:botID/edit", checkMaintence, checkAuth, async (req,res) => {
      ...
      client.channels.cache.get(channels.botlog).send(`<@${req.user.id}> edited <@${req.params.botID}>`)
    ...
    app.get("/bot/:botID/delete", async (req, res) => {
      ...
      app.get("/admin/confirm/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
         ...
         client.users.fetch(req.params.botID).then(bota => {
              client.channels.cache.get(channels.botlog).send(`<:bot_verify:845721551550545962>  <@${botdata.ownerID}>'s Your Bot <@${req.params.botID}> Has Been Approved`)
              client.users.cache.get(botdata.ownerID).send(`<:bot_verify:845721551550545962> Your bot named **${bota.tag}** has been approved.`)
         });
      ...
      app.post("/admin/decline/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
          let rBody = req.body;
           ...
           client.users.fetch(botdata.ownerID).then(sahip => {
               client.channels.cache.get(channels.botlog).send(`<:remove:845717736692908042> <@${botdata.ownerID}>'s your bot \`${botdata.username}\` has been declined`)
               client.users.cache.get(botdata.ownerID).send(`<:remove:845717736692908042> Your bot named **${botdata.username}** has been declined.\nReason: **${rBody['reason']}**\nAuthorized: **${req.user.username}**`)
          })
          await botsdata.deleteOne({ botID: req.params.botID, ownerID: botdata.ownerID })
     ...
     app.get("/admin/premium/delete/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
       ...
       app.get("/admin/certificate/give/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
         ...
         let botdata = await botsdata.findOne({ botID: req.params.botID });
          ...
          client.users.fetch(botdata.botID).then(bota => {
              client.channels.cache.get(channels.botlog).send(`<:verify:845717753884442634> <@${botdata.ownerID}>'s your bot <@${req.params.botID}> has been granted a certificate.`)
              client.users.cache.get(botdata.ownerID).send(`<:verify:845717753884442634> Your bot named **${bota.tag}** has been certified.`)
          });
       ...
       app.post("/admin/certificate/delete/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
         ...
         let botdata = await botsdata.findOne({ botID: req.params.botID });
          client.users.fetch(botdata.botID).then(bota => {
              client.channels.cache.get(channels.botlog).send(`<:unverified:845736611479945226> <@${botdata.ownerID}>'s your bot <@${req.params.botID}> has not been granted a certificate.`)
       ...
       app.post("/admin/editcode/:code", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
          ...
          .setDescription(`The user named **[${req.user.username}](https://disbotlist.xyz/user/${req.user.id})** edited the code named **${rBody['codename']}**.`)
          .addField("Code Link", `https://disbotlist.xyz/code/${kod}`, true)
          .addField("Code Description", rBody['codedesc'], true)
          .addField("Code Category", rBody['category'], true)
          )
      ...
      app.post("/admin/maintence", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.channels.cache.get(channels.webstatus).send(`<a:maintence:833375738547535913> DisBot has been switched to __maintance__ due to **${req.body.reason}**`).then(a => { 
      ...
      app.post("/admin/unmaintence", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        .setThumbnail(client.user.avatarURL())
        .setColor("GREEN")
        .setDescription(`<:online:849501680952541204> DisBot are **active** again!\n[Click to redirect website](https://disbotlist.xyz)`)
        .setFooter("DisBot © All rights reserved.");
        await client.channels.cache.get(channels.webstatus).messages.fetch(bakimdata.bakimmsg).then(a => { a.edit(`~~ <a:maintence:833375738547535913> DisBot has been switched to __maintance__ due to **${bakimdata.reason}** ~~`, bakimsonaerdikardesvcodes) } )
  ...
  await profiledata.findOneAndUpdate({userID: req.user.id}, {$set: {instagram: rBody['instagram']}}, {upsert:true})
   ...
   //------------------- API  -------------------//
    app.get("/api", async (req, res) => {
      res.json({ "Hello": "World", "Template by": "disbotlist . x y z"});
    });
